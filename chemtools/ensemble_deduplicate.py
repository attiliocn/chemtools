...
from modules.geometry import rmsd_matrix, get_duplicates_rmsd_matrix
...
for file in args.files:
    ...
    rmsd_distance_matrix = rmsd_matrix(coordinates_all)
    to_delete = get_duplicates_rmsd_matrix(rmsd_distance_matrix, threshold=args.threshold)
    ...
    with open('deduplicate.log', mode='w') as f:
        f.write(f"Current file: {basename}\n")
        f.write(f"Number of conformers: {numconfs}\n")
        f.write(f"Conformers after deduplication: {len(_coordinates)}\n")
        f.write(f"{numconfs - len(_coordinates)} conformers were deleted\n")
        f.write(f"{len(_coordinates)} conformers will be written to {basename_updated}\n\n")
